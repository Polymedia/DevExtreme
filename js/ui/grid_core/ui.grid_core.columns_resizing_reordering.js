    ...
    width: function(value) {
        ...
        if($element) {
            if(isDefined(value)) {
                ...
                return $element.width() * getScaleCorrector().scale();
            }
        }
    }
});
...
const ColumnsResizerViewController = modules.ViewController.inherit({
    ...
    _moveSeparator: function(args) {
        ...
        if(that._isResizing && that._resizingInfo) {
            ...
            if((horizontalShift(parentOffsetLeft) <= horizontalShift(eventData.x) || !isNextColumnMode && isRtlParentStyle) && (!isNextColumnMode || horizontalShift(eventData.x) <= horizontalShift(parentOffsetLeft) + that._$parentContainer.width())) {
